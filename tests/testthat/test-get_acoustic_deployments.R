...
test_that("get_acoustic_deployments() allows selecting on deployment_id", {
  # Errors
  expect_error(get_acoustic_deployments(credentials, deployment_id = "not_a_deployment_id"))
  expect_error(get_acoustic_deployments(credentials, deployment_id = c("1437", "not_a_deployment_id")))
...
test_that("get_acoustic_deployments() allows selecting on receiver_id", {
  # Errors
  expect_error(get_acoustic_deployments(credentials, receiver_id = "not_a_receiver_id"))
  expect_error(get_acoustic_deployments(credentials, receiver_id = c("VR2W-124070", "not_a_receiver_id")))
...
test_that("get_acoustic_deployments() allows selecting on acoustic_project_code", {
  # Errors
  expect_error(get_acoustic_deployments(credentials, acoustic_project_code = "not_a_project"))
  expect_error(get_acoustic_deployments(credentials, acoustic_project_code = c("demer", "not_a_project")))
...
test_that("get_acoustic_deployments() allows selecting on station_name", {
  # Errors
  expect_error(get_acoustic_deployments(credentials, station_name = "not_a_station_name"))
  expect_error(get_acoustic_deployments(credentials, station_name = c("de-9", "not_a_station_name")))
...
test_that("get_acoustic_deployments() allows selecting on open deployments only", {
  # Errors
  expect_error(get_acoustic_deployments(credentials, open_only = "not_a_logical"))

  # ws1 is an open ended acoustic project
  all_df <- get_acoustic_deployments(credentials, acoustic_project_code = "ws1", open_only = FALSE)

  # Default returns all
  default_df <- get_acoustic_deployments(credentials, acoustic_project_code = "ws1")
  expect_equal(default_df, all_df)

  # Open only returns deployments with no end date
  open_only_df <- get_acoustic_deployments(credentials, acoustic_project_code = "ws1", open_only = TRUE)
...
test_that("get_acoustic_deployments() does not return cpod deployments", {
  ...
  df <- get_acoustic_deployments(credentials, receiver_id = "C-POD-408")
