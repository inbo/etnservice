...
test_that("get_acoustic_detections() allows selecting on start_date and end_date", {
  # Errors
  expect_error(get_acoustic_detections(credentials, start_date = "not_a_date"))
  expect_error(get_acoustic_detections(credentials, end_date = "not_a_date"))

  # 2014_demer contains data from 2014-04-18 15:45:00 UTC to 2018-09-15 19:40:51 UTC

  # Start date (inclusive) <= min(date_time)
  start_year_df <- get_acoustic_detections(credentials, start_date = "2015", animal_project_code = "2014_demer")
  expect_lte(as.POSIXct("2015-01-01", tz = "UTC"), min(start_year_df$date_time))
  start_month_df <- get_acoustic_detections(credentials, start_date = "2015-04", animal_project_code = "2014_demer")
  expect_lte(as.POSIXct("2015-04-01", tz = "UTC"), min(start_month_df$date_time))
  start_day_df <- get_acoustic_detections(credentials, start_date = "2015-04-24", animal_project_code = "2014_demer")
  expect_lte(as.POSIXct("2015-04-24", tz = "UTC"), min(start_day_df$date_time))

  # End date (exclusive) > max(date_time)
  end_year_df <- get_acoustic_detections(credentials, end_date = "2016", animal_project_code = "2014_demer")
  expect_gt(as.POSIXct("2016-01-01", tz = "UTC"), max(end_year_df$date_time))
  end_month_df <- get_acoustic_detections(credentials, end_date = "2015-05", animal_project_code = "2014_demer")
  expect_gt(as.POSIXct("2015-05-01", tz = "UTC"), max(end_month_df$date_time))
  end_day_df <- get_acoustic_detections(credentials, end_date = "2015-04-25", animal_project_code = "2014_demer")
  expect_gt(as.POSIXct("2015-04-25", tz = "UTC"), max(end_day_df$date_time))

  # Between
  between_year_df <- get_acoustic_detections(credentials, start_date = "2015", end_date = "2016", animal_project_code = "2014_demer")
  ...
  between_month_df <- get_acoustic_detections(credentials, start_date = "2015-04", end_date = "2015-05", animal_project_code = "2014_demer")
  ...
  between_day_df <- get_acoustic_detections(credentials, start_date = "2015-04-24", end_date = "2015-04-25", animal_project_code = "2014_demer")
...
test_that("get_acoustic_detections() allows selecting on acoustic_tag_id", {
  # Errors
  expect_error(get_acoustic_detections(credentials, acoustic_tag_id = "not_a_tag_id"))
  expect_error(get_acoustic_detections(credentials, acoustic_tag_id = c("A69-1601-16130", "not_a_tag_id")))
...
test_that("get_acoustic_detections() allows selecting on animal_project_code", {
  # Errors
  expect_error(get_acoustic_detections(credentials, animal_project_code = "not_a_project"))
  expect_error(get_acoustic_detections(credentials, animal_project_code = c("2014_demer", "not_a_project")))
  ...
  expect_equal(
    get_acoustic_detections(credentials, animal_project_code = "2014_demer", limit = TRUE),
    get_acoustic_detections(credentials, animal_project_code = "2014_DEMER", limit = TRUE)
  )
...
test_that("get_acoustic_detections() allows selecting on scientific_name", {
  # Errors
  expect_error(get_acoustic_detections(credentials, scientific_name = "not_a_sciname"))
  expect_error(get_acoustic_detections(credentials, scientific_name = "rutilus rutilus")) # Case sensitive
  expect_error(get_acoustic_detections(credentials, scientific_name = c("Rutilus rutilus", "not_a_sciname")))
...
test_that("get_acoustic_detections() allows selecting on acoustic_project_code", {
  # Errors
  expect_error(get_acoustic_detections(credentials, acoustic_project_code = "not_a_project"))
  expect_error(get_acoustic_detections(credentials, acoustic_project_code = c("demer", "not_a_project")))
  ...
  # Selection is case insensitive
  expect_equal(
    get_acoustic_detections(credentials, acoustic_project_code = "demer", limit = TRUE),
    get_acoustic_detections(credentials, acoustic_project_code = "DEMER", limit = TRUE)
  )
...
test_that("get_acoustic_detections() allows selecting on receiver_id", {
  # Errors
  expect_error(get_acoustic_detections(credentials, receiver_id = "not_a_receiver_id"))
  expect_error(get_acoustic_detections(credentials, receiver_id = c("VR2W-124070", "not_a_receiver_id")))
...
test_that("get_acoustic_detections() allows selecting on station_name", {
  # Errors
  expect_error(get_acoustic_detections(credentials, station_name = "not_a_station_name"))
  expect_error(get_acoustic_detections(credentials, station_name = c("de-9", "not_a_station_name")))
...
test_that("get_acoustic_detections() allows to limit to 100 records", {
  # Errors
  expect_error(get_acoustic_detections(credentials, limit = "not_a_logical"))

  # Limit
  expect_equal(nrow(get_acoustic_detections(credentials, limit = TRUE)), 100)
  expect_equal(
    nrow(get_acoustic_detections(credentials, acoustic_project_code = "demer", limit = TRUE)),
    100
  )
})
...
test_that("get_acoustic_detections() returns acoustic and acoustic-archival tags", {
  acoustic_df <- get_acoustic_detections(credentials, acoustic_tag_id = "A69-1601-16130")
...
test_that("get_acoustic_detections() returns detections from acoustic_tag_id_alternative", {
  ...
  expect_gt(nrow(get_acoustic_detections(credentials, acoustic_tag_id = "A69-1105-26")), 0)
  ...
  expect_equal(nrow(get_acoustic_detections(credentials, acoustic_tag_id = "A69-1105-155")), 0)
})
...
test_that("get_acoustic_detections() does not return duplicate detections when tags are reused", {
  ...
  df_both <- get_acoustic_detections(credentials, acoustic_tag_id = "A69-1601-29925")
  df_393 <- get_acoustic_detections(credentials, acoustic_tag_id = "A69-1601-29925", start_date = "2012-12-01", end_date = "2012-12-10")
...
test_that("get_acoustic_detections() does not return detections out of date range when tag is associated with animal", {
  ...
  in_range_df <- get_acoustic_detections(credentials, acoustic_tag_id = "A69-1303-20695", start_date = "2010-08-09", end_date = "2011-05-19")
  pre_range_df <- get_acoustic_detections(credentials, acoustic_tag_id = "A69-1303-20695", end_date = "2010-08-09")
  post_range_df <- get_acoustic_detections(credentials, acoustic_tag_id = "A69-1303-20695", start_date = "2011-05-19")
...
test_that("get_acoustic_detections() can return detections not (yet) associated with an animal", {
  ...
  expect_gt(nrow(get_acoustic_detections(credentials, acoustic_tag_id = "A180-1702-49684")), 0)
})
